...
# ca-certificates: a imagem "slim" nao vem com o bundle de certificados
# raiz — sem isso o curl abaixo falha (exit 77) ao tentar validar o HTTPS
# do GitHub para baixar o modelo de portugues.
...
RUN TESSDIR=$(find / -maxdepth 6 -type d -name tessdata 2>/dev/null | head -n1) \
    && curl -fsSL -o "$TESSDIR/por.traineddata" \
